...
cp ../../intermediate_ca_openssl.cnf cnf
:>| index.txt
echo 1000 > serial
echo 1000 > crlnumber
cd ../
...
openssl req -config Root/cnf/root_ca_openssl.cnf -key Root/private/RootCA.key.pem -new \
...
read -p "Press Enter to Continue..." </dev/tty
openssl req -config Intermediate/cnf/intermediate_ca_openssl.cnf -new -sha512 \
    -key Intermediate/private/intermediate.key.pem -out Intermediate/csr/Intermediate.csr.pem

openssl ca -config Root/cnf/root_ca_openssl.cnf -extensions v3_intermediate_ca -days 365 \
    -md sha512 -in Intermediate/csr/Intermediate.csr.pem \
    -out Intermediate/certs/Intermediate.cert.pem

openssl x590 -noout -text -in Intermediate/certs/Intermediate.cert.pem
read -p "Press Enter to Continue..." </dev/tty
openssl verify -CAfile Root/certs/RootCA.cert.pem Intermediate/certs/Intermediate.cert.pem
read -p "Press Enter to Continue..." </dev/tty
cat Intermediate/certs/Intermediate.cert.pem Root/certs/RootCA.cert.pem > \
    Intermediate/certs/ca-chain.cert.pem

openssl ca -config Intermediate/cnf/intermediate_ca_openssl.cnf -gencrl -out \
    Intermediate/crl/Intermediate.crl.pem

cat Intermediate/certs/Intermediate.cert.pem Intermediate/crl/Intermediate.crl.pem \
